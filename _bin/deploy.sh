...
if [[ $# -ne 1 ]]; then
  DST="$BIN/../../trustin.github.com"
  if [[ ! -d "$DST/.git" ]]; then
    echo 'Specify the path to git@github.com:trustin/trustin.github.com.git'
    exit 1
  fi
else
  DST="$1"
...
# Make sure trustin-website is clean
pushd "$BIN/.."
#ensure_clean_copy
popd

# Make sure trustin.github.com is clean
pushd "$DST"
if ! git remote -v | grep -q -E '(git@|https://)github.com[:/]trustin/trustin.github.com'; then
  echo "Not a trustin.github.com repository: $PWD"
  exit 1
fi
ensure_clean_copy
popd
...
# Copy the generated web site to trustin.github.com
